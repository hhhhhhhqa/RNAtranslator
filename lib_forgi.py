...
class BulgeGraph(object):
    ...
    def ss_distance(self, e1, e2):
        ...
        for key, group in it.groupby(enumerate(self.define_residue_num_iterator(e1, adjacent=True)), lambda index, item: index - item):
            group = map(oper.itemgetter(1), group)
            d1_corners += group

        for key, group in it.groupby(enumerate(self.define_residue_num_iterator(e2, adjacent=True)),lambda index, item: index - item):
            group = map(oper.itemgetter(1), group)
            d2_corners += group
